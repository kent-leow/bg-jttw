...
**Input**: problem description → **Output**: `report.md` + `flowchart.mmd` in `.docs/<topic>/`

## Constraints

- Every claim must cite file, log line, code path, or terminal output
- Read-safe commands only (searches, log tails, test runs); ask before writes/destructive ops
- Flowchart required alongside report

---

## Phase 1 — Understand

- DO: parse **What** (symptom) · **When** (trigger) · **Where** (service/component) · **Impact**
- IF: missing critical context → ask up to 3 targeted questions

## Phase 2 — Plan

- DO: create investigation plan via `todo` — testable hypotheses + verification steps
...
- LOOP: each root cause → EMIT: immediate mitigation, permanent fix, trade-offs

## Phase 6 — Write Outputs

- STORE: folder = `.docs/<kebab-topic>/`
- DO: write `report.md`:
